...
基于 **LangGraph + FastAPI + 大模型** 的企业级智能助手：打通私有知识库检索与企业内部业务系统，
以**五个职责单一的子 Agent**（路由 / 闲聊 / 简单 RAG / 复杂 RAG / 工具）协作完成意图识别、
边界管控、知识问答、业务工具调用、多轮对话与异常兜底的全链路闭环。

> **零配置就能跑**：没有 API Key、没有 Redis、没有向量库时，服务仍完整启动并返回合理答案
> （Mock 模型 + 本地哈希向量）。「先把环境跑起来」和「接上真实模型」是两件可以分开做的事。

<p align="center"><img src="docs/images/panel-answer.png" alt="智能对话：流式输出 + 引用溯源 + 场景徽章" width="860" /></p>

**目录**：[一、快速开始](#一快速开始) · [二、功能展示](#二功能展示) · [三、核心能力](#三核心能力) ·
[四、用法示例](#四用法示例) · [五、项目结构](#五项目结构) ·
[六、LangGraph 工作流](#六langgraph-工作流五-agent-协作) · [七、API 接口清单](#七api-接口清单) ·
[八、关键设计说明](#八关键设计说明) · [九、配置说明](#九配置说明) · [十、实测结果](#十实测结果) ·
[十一、生产部署与运维](#十一生产部署与运维) · [十二、二次开发与贡献](#十二二次开发与贡献)
...
浏览器打开 <http://localhost:8001> 即是可视化面板。

### 验证部署是否成功

```bash
curl -s http://localhost:8001/health                # 服务状态 + 向量库 + 索引条数
curl -s http://localhost:8001/test/quick            # 5 项快速自检
curl -s http://localhost:8001/knowledge/list        # 知识库：文档与片段总数
```

首次启动会自动完成两件事：**本地轻量自检**（探活基础设施，不真实调用 LLM，避免启动被上游
网络拖住）、**知识库为空时自动建索引**。所以放进 `data/` 的文档无需手工触发重建；
改了文档再调 `POST /knowledge/rebuild`。
...
默认编排只含 **应用 + Redis**。要用 **Milvus** 时用 profile 按需拉起 Milvus Standalone——
它会一并启动所依赖的 **etcd** 与 **minio**，共三个容器（Milvus 自己不存元数据与对象，
单起一个 milvus 容器是起不来的）：
...
（把 `WITH_VECTOR_CLIENTS=1`、`VECTOR_DB_TYPE=milvus` 写进 `.env` 后即可省略前缀。）
...
（路由判定的意图 / 命中的知识域 / 耗时），以及点赞点踩反馈。截图见本文开头。

### 2.2 可观测性：执行耗时瀑布图

每次请求产出一棵**嵌套 span 树**（对齐 OpenTelemetry 的 trace=span 树），
面板按耗时比例画出瀑布图，能直接看出"这一轮到底慢在哪一步"。
下图可以直接读到：总耗时 5331ms 里 `generate_answer` 占 4414ms，
而路由只花了 14ms——这是本地意图漏斗的效果。

<p align="center"><img src="docs/images/panel-trace.png" alt="执行耗时瀑布图：嵌套 span 树" width="780" /></p>

### 2.3 工作流引擎

LangGraph 拓扑可视化：**9 个节点 / 3 条条件边**，路由 Agent 是唯一入口。
面板会展示编译图的节点集合，改了图却忘了同步节点标签会在这里露出来。

<p align="center"><img src="docs/images/panel-workflow.png" alt="工作流拓扑：9 节点 / 3 条件边" width="780" /></p>
...
<p align="center"><img src="docs/images/panel-knowledge.png" alt="知识库管理：文档与片段" width="780" /></p>
...
| 服务自测 | 一键跑 9 项全量自检（含真实调用 LLM / Embedding）或 5 项快速自检 |
...
| Rerank 精排（可选） | cross-encoder 二次评分重排，与 reorder 正交。两种后端按配置自动选：配了 `RERANK_BASE_URL` + Key 走 `/rerank` 接口，否则回退本地 `sentence-transformers`。**默认关闭** —— 实测在本项目用例集上三项指标全部下降（见 [§十](#十实测结果)），开启前建议先跑 `scripts/eval_retrieval.py` 自己验一次 |
| 请求级上下文 | 同请求内共享中间结果，避免重复计算与参数穿透（`app/core/request_ctx.py`）：query 向量复用、来源白名单、本轮证据。**授权事实与证据不由模型回传**——否则等于把"我能查谁的资料"交给模型决定 |
| 可观测性 | 嵌套 span 树 + 前端瀑布图 + `logs/trace.jsonl` 持久化（**落盘前统一过一层脱敏**，见 `TRACE_MASK_*`）；可选接入 LangSmith（`LANGSMITH_*` 环境变量） |
| 工程化横切 | 统一异常基类、Prompt 注册表、trace_id 贯穿日志、入站限流（滑动窗口，默认 20 次/分/IP）、可选入站鉴权与 CORS |
...
| Dify 兼容 | 实现 Dify 的 External Knowledge API，可被 Dify 的「知识检索」节点直接调用——**文档不用再往 Dify 传一份**，切分策略与拒答阈值仍由本项目掌控。见 [§4.5](#45-把本项目接入-dify-作为外部知识库) |
| 全链路自测 | 启动执行本地轻量自检（不真实调用模型）；9 项全量检查（真实调用 LLM/Embedding）经 `/test/all` 触发；另提供 615 项 pytest 回归（含 80 项多 Agent 架构级用例） |

---

## 四、用法示例

下面每条都对应一次**真实调用**（本机实测输出，非示意）。

### 4.1 知识问答

```bash
curl -s -X POST http://localhost:8001/chat/ask \
  -H 'Content-Type: application/json' \
  -d '{"query":"年假有多少天？","session_id":"demo","user_id":"u1"}'
```
...
{
  "code": 0,
  "answer": "年休假天数根据累计工作年限确定[1]：\n\n- 累计工作满 1 年不满 10 年：5 天[1]\n- 满 10 年不满 20 年：10 天[1]\n- 满 20 年：15 天[1]\n\n依据：《员工手册》第三章 假期管理 > 3.2 年休假[1]。",
  "scene": "simple_rag",
  "scene_reason": "本地漏斗（lexical）：词面通过：policy_single（领先 0.500）",
  "scene_source": "router:local",
  "intent": "knowledge",
  "intent_source": "simple_rag",
  "confidence": 1.0,
  "refused": false,
  "truncated": false,
  "citations": [
    {"index": 1, "source": "data/员工手册.txt", "file_name": "员工手册.txt",
     "score": 0.6101, "lexical": 1.0, "fallback": false}
  ],
  "elapsed_ms": 6768,
  "llm_mode": "real"
}
...
- `scene_reason` 说明**这次判定是怎么来的**——`router:local` 表示零模型本地漏斗就判定了，
  **没有消耗一次模型调用**；判不了时才会是模型路由。
- `scene` 与 `intent` 是**两个不同的问题**：前者是"路由判给谁"，后者是"答案实际怎么来的"。

### 4.2 流式对话

```bash
curl -N -X POST http://localhost:8001/chat/ask/stream \
  -H 'Content-Type: application/json' \
  -d '{"query":"年假有多少天？","session_id":"demo","user_id":"u1"}'
```

SSE 帧序列（真实输出，`token` 帧已截断）：

```
event: stage
data: {"scene":"simple_rag","scene_source":"router:local","intent":"knowledge",
       "agent_steps":[{"node":"simple_rag","kind":"retrieve","detail":"命中 5 条"}],
       "hits":5,"tool_result":null}
...
event: meta
data: {"citations": [...], "confidence": 1.0, "elapsed_ms": 5331, ...}
```

帧序为 `stage`（路由与检索进度）→ `token`（逐字）→ `meta`（来源 / 引用 / 耗时）→ `done`。
**反代部署时务必关掉缓冲**，否则流式会被攒成一次性响应（见 [§11.2](#112-反向代理必须关掉缓冲)）。

### 4.3 业务工具调用（function calling）

```bash
curl -s -X POST http://localhost:8001/chat/ask \
  -H 'Content-Type: application/json' \
  -d '{"query":"张三在哪个部门？","session_id":"demo2"}'
```
...
```json
{
  ...
  "scene": "tool",
  "intent": "tool",
  "intent_capability": "find_employee_by_name,query_employee_info",
  "agent_steps": [
    {"index": 0, "tool": "find_employee_by_name", "status": "ok", "elapsed_ms": 2},
    {"index": 1, "tool": "query_employee_info", "status": "ok", "elapsed_ms": 3}
  ]
}
```

`intent_capability` 里是**两个工具**：模型自己完成了**链式调用**（先按姓名换工号，
再用工号查详情），代码里没有写这条链。

### 4.4 路由预演：这句话为什么走了这条路

路由是一个**可判错的粗分类器**，判错的排障成本极高。"为什么被分到工具通道"如果只能靠读日志
反推，就永远无法回答"是分低了，还是阈值配错了"。这个接口把完整判据摊开：

```bash
curl -s -X POST http://localhost:8001/routing/intent-preview \
  -H 'Content-Type: application/json' \
  -d '{"query":"张三在哪个部门？","explain":true}'
```
...
```json
{
  ...
  "source": "anchor",
  "reason": "锚点：人属性索取句式（对象=张三）",
  "elapsed_ms": 5,
  "thresholds": {
    "embedding_mode": "api",
    "lexical_floor": 0.45, "lexical_margin": 0.25,
    "semantic_floor": 0.35, "semantic_margin": 0.03,
    "budget_ms": 2500
  }
}
```

它返回**候选得分表与本次生效的阈值**，而不是一个标签。接口是**只读预演**：
不写请求上下文、不调子 Agent、不落库，因此可以在生产流量上做影子调用。

### 4.5 把本项目接入 Dify 作为外部知识库

配好 `DIFY_API_KEY` 后，Dify 的「知识检索」节点可以直接调用本项目：

```bash
# 接入前先自查（不返回密钥）
curl -s http://localhost:8001/dify/info
```
...
```json
{
  ...
  "knowledge_id": "（未限制，接受任意非空值）",
  "index_chunks": 176,
  "score_range": "0~1（已对 RRF 融合分做归一化）"
}
```

Dify 侧填 `API Base URL` + `API Key` 即可，两种端点填法都支持（`/retrieval` 与
`/dify/retrieval` 同时注册，避免猜该填哪个）。完整契约与踩坑记录见
...
│   │   ├── workflow.py         #   工作流管理接口（拓扑声明唯一来源在此）
│   │   ├── routing.py          #   意图路由预演接口（只读）
│   │   ├── dify.py             #   Dify 外部知识库兼容接口
│   │   └── test.py             #   服务自测接口
...
│   │   ├── structure.py        #   L1.5 结构解析：章节锚点识别、heading_path 拼接
│   │   ├── retriever.py        #   L3 检索优化：向量+词面多路并发召回、RRF 融合
...
│   │   ├── routing/            #   意图路由：本地漏斗（锚定/词面）、词表派生、目录
...
│   ├── tools/                  # 业务工具（全部只读）
│   │   └── sqlite_tools.py     #   Function Calling 工具（含 JSON Schema 定义）
│   ├── utils/                  # 通用工具（依赖树的**最底层**，不得反向依赖上层）
│   │   ├── logger.py           #   全链路日志（含 trace_id 注入）
│   │   ├── doc_loader.py       #   多格式文档解析（PDF 三级降级）
│   │   ├── auth_header.py      #   Bearer / X-API-Key 解析与比对
...
├── tests/                      # pytest 回归套件（615 项）
│   ├── test_multi_agent.py     #   多 Agent 架构级用例（80 项，守"谁来做决定"）
│   ├── test_topology_declaration.py  # 拓扑声明 vs 编译图（面板/日志/BRANCHES 三方对齐）
│   ├── test_sqlite_tools.py    #   只读工具 + 只读强制 + 边界情形
│   ├── test_layering.py        #   分层契约（import-linter，含"契约真的会红"自检）
│   ├── test_doc_linenos.py     #   文档行号校验器自身的回归（逐类反向验证）
│   └── test_service.py         #   端到端脚本（4 阶段 15 项，需起服务）
...
│   ├── check_vector_db.py      #   向量库连通性自检（写入→检索→删除往返）
...
└── .env.example                # 配置模板（**每一项都带中文注释说明为什么这么定**）
```

---

## 六、LangGraph 工作流（五 Agent 协作）
...
| Agent | 输入 | 输出 | 模型调用 |
|---|---|---|---|
| **路由** | 用户原话 | `scene` + 判定理由 + 来源 | 1 次（本地漏斗命中则 **0 次**） |
| **闲聊** | 用户原话 | 模板回复 | **0 次** |
| **简单 RAG** | 用户原话 | 证据片段 + 答案 | 检索 1 次 |
| **复杂 RAG** | 用户原话 | 拆解后的多个子问题 + 合并证据 | 1 次（拆解）+ 多次检索 |
| **工具** | 用户原话 + 历史 | 工具结果 / 追问话术 | 1~N 次（链式调用） |

**两个编译产物共用同一个装配函数**（`build_workflow_graph` 9 节点 / `build_pre_generation_graph`
8 节点），唯一差别是"证据出口通向 `generate_answer` 还是 `END`"——流式链路**不可能**重抄一遍
业务逻辑。
...
> 详细设计（为什么拆、`scene` 与 `intent_type` 的分工、失败处置的统一判据）
> 见 [`docs/multi-agent-architecture.md`](docs/multi-agent-architecture.md)；
> 工具的完整实现与 Function Calling JSON Schema 见
> [`docs/tool-json-schema.md`](docs/tool-json-schema.md)。
...
| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/workflow/status` | 工作流状态与拓扑（节点 / 条件边声明） |
| POST | `/workflow/execute` | 手动触发工作流执行（不写会话历史，用于调试） |
| POST | `/routing/intent-preview` | **路由预演**：返回候选得分与本次生效阈值，只读无副作用 |
| GET | `/routing/catalog` | 意图目录：通道 / 能力 / 例句一览 |
...
| PUT | `/memory/soul` | 覆盖写入**全局**人格设定（`SOUL.md`）。⚠️ 影响面是所有人，故**要求开启入站鉴权** |
...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | `/retrieval` · `/dify/retrieval` | Dify External Knowledge API 契约入口（两种填法都支持） |
| GET | `/dify/openapi.json` | 供 Dify 自定义工具导入的 OpenAPI schema |
| GET | `/dify/info` | 接入自检探针：返回配置状态，**不含密钥**，免鉴权 |

### 服务自测

| 方法 | 路径 | 说明 |
|---|---|---|
| GET | `/test/all` | 全量服务测试（9 项，含深度检查：真实调用 LLM / Embedding） |
| GET | `/test/quick` | 快速健康检测（5 项，不真实调用模型） |

交互式文档：<http://localhost:8001/docs>（共 31 个端点）
...
「已降级（配置 xxx）」，`/health` 里 `vector_db_requested` 与 `vector_db` 不一致即说明发生了降级。
...
- **入站鉴权**（`AUTH_ENABLED`）：开启后除豁免路径外必须携带 `X-API-Key` 或
  `Authorization: Bearer`。**fail-closed**——开了鉴权却没配 Key 时一律拒绝，
  而不是放行（"开着鉴权但谁都能过"比不开更危险）
- **来源访问控制**（`SOURCE_ACL`）：把 `user_id` 映射到允许访问的文档来源白名单，
  服务端解析对客户端传入的 `allowed_sources` 具有权威性
- 输入注入过滤：清除 `<script>` `{{ }}` `<% %>` `javascript:` 等片段
- 参数长度限制：提问 ≤ 2000 字符，单篇文档 ≤ 50000 字符
- 文件名安全清洗：替换非法字符，禁止隐藏文件入库
- 会话 24 小时自动过期；API Key 在日志与接口中**只显示首尾各 4 位**
- `logs/trace.jsonl` 落盘前统一脱敏（`TRACE_MASK_*`）

---

## 九、配置说明

全部配置项见 [`.env.example`](.env.example)，每一项都带中文注释说明为什么这么定。
只列几个最容易配错的：
...
| `LLM_TEMPERATURE` | 0.1 | **必须与 `LLM_DISABLE_THINKING` 配套**：部分模型关思考后只接受特定温度，配错会被接口以 400 明确拒绝（见 `app/config.py` 对应注释） |
| `LLM_MAX_TOKENS` | 1024 | **推理模型的思考 token 与正文共享该额度**，1024 会让正文写到一半被截断。用推理模型请留足余量（建议 ≥4096） |
| `LLM_DISABLE_THINKING` | false | 关闭推理模型的「思考」阶段。实测同一问句：思考开启首个可见字 11123ms / 关闭 1137ms，相差约 10 倍 |
| `LLM_HEALTH_CHECK` | false | 启动时是否对真实模型做 ping 校验。默认关是**可用性优先**——启动时上游不通不该让服务起不来 |
...
| `SCORE_THRESHOLD` | 自适应 | 余弦相似度阈值，留空按 embedding 模式自动选择（见 [§八.3](#3-检索阈值按-embedding-模式自适应)） |
...
| `CHUNK_STRATEGY` | structure | `structure`（结构感知）/ `recursive`。⚠️ **改任一切分开关后必须全量重建索引**——`chunk_id` 是内容哈希，新旧产物 ID 不同，增量入库会残留旧块 |
| `RERANK_ENABLED` | false | 是否启用精排。**默认关闭且有实测依据**（见 [§十](#十实测结果)） |
...
| `AUTH_ENABLED` / `AUTH_API_KEY` | false / 空 | 入站鉴权与密钥。开启后 `AUTH_API_KEY` **必填**（fail-closed） |
| `CORS_ORIGINS` | * | 允许的来源。**部署到具体域名时应显式列出**；为通配时凭据自动关闭（浏览器规范要求） |
| `RATE_LIMIT_PER_MINUTE` | 20 | 单 IP 每分钟入站上限。⚠️ 填 0 **不是**"不限流"，会让所有请求被拒——需要放开请填大数 |
| `DIFY_API_KEY` | 空 | Dify 外部知识库密钥。留空 = 端点拒绝服务（安全默认） |

---

## 十、实测结果

环境：Python 3.13 / macOS，API 模式（真实 Embedding + 真实大模型），12 篇文档 → 176 条向量片段。

```
全量服务测试 /test/all：9/9 通过（3.0s，含真实调用 LLM / Embedding）
  服务健康状态 · 知识库资源 · 大模型配置 · 向量库与索引 · Agent 工具与路由配置
  业务数据库与工具 Schema · 语义检索能力 · LangGraph 工作流 · 端到端对话链路

快速自检 /test/quick：5/5 通过

端到端全链路 tests/test_service.py：15/15 通过（4 阶段）
  阶段 1 服务健康 3/3 · 阶段 2 知识库 5/5
  阶段 3 工作流 2/2 · 阶段 4 对话链路 5/5

回归套件：615 passed（含文档行号校验、分层契约、死代码扫描）
索引一致性：向量库 176 条 / 词面倒排索引 176 个键，两处来源集合一致
```
...
默认配置是为了**本地零依赖可跑**，不是为生产准备的。下面几项需要逐项确认：

| 项 | 默认 | 生产建议 | 不做会怎样 |
|---|---|---|---|
| `AUTH_ENABLED` + `AUTH_API_KEY` | `false` / 空 | 开启并配强随机 Key | 接口对公网裸奔；`PUT /memory/soul` 会直接拒绝服务 |
| `CORS_ORIGINS` | `*` | 显式列出域名 | 任意站点可带着用户凭据调用你的接口 |
...
| `REDIS_HOST` | 空 | 指向 Redis | 会话历史降级为进程内字典，重启即丢 |
| `TRACE_MASK_ENABLED` | `true` | 保持 `true` | trace 原文（可能含提问正文）落盘 |
| `RATE_LIMIT_PER_MINUTE` | 20 | 按业务调整 | 单 IP 限流，多副本时会被放大 N 倍（见 [§11.3](#113-部署形态默认单实例)） |
| 反向代理 | 无 | Nginx / Caddy 反代 8001 | SSE 流式响应须关掉代理缓冲（见下） |
...
| 流式输出不逐字、一次性返回 | 反向代理的 `proxy_buffering`（见 [§11.2](#112-反向代理必须关掉缓冲)） |
...
| 「某条知识怎么问都检索不到」 | 跑一次索引对账（向量库与词面索引的来源集合是否一致）：`python -c "from app.rag.indexer import reconcile; print(reconcile())"` |
| 连不上本机 Milvus，报 `server unavailable` | gRPC 会读 `http_proxy`：把本机地址加进 `NO_PROXY`（`scripts/check_vector_db.py` 会明确提示这一点） |

### 11.5 运行测试与门禁

```bash
# 主回归套件（615 项，纯 pytest，不需要起服务）
pytest                                   # 全量
pytest tests/test_multi_agent.py -q      # 只跑多 Agent 架构级用例（80 项）
pytest -k tool_agent                     # 按名字筛选

# 端到端脚本（需先起服务；4 阶段 15 项，走真实 HTTP）
...
1. **分层方向**：`api → graph → core → {rag, memory, tools} → {db, providers} → utils`。
   ...
   豁免清单**只减不增**）。例外：允许能力层用 `app/core/` 里的**基础设施**子模块
   （prompts / llm_factory / tracing …）。
...
3. **文档里的行号与界面上的拓扑声明必须与代码一致**：`docs/project-introduction.md`
   是一份带精确行号的代码地图；`app/static/index.html` 的面板横幅、
   `app/api/workflow.py` 的 `BRANCHES`、启动日志三处的节点/条件边数由
   `tests/test_topology_declaration.py` 钉在编译图上。改完 `app/` 下任何文件都要跑
   `python scripts/fix_doc_linenos.py --write`。
...
模型就是靠它抽参数的。然后把工具加进同文件的 `TOOL_AGENT_TOOLS` 元组即可。两个容易漏的配套：
...
**让路由认识新说法**：意图路由的词表是从每个意图的 `utterances`（例句）**反推**出来的，
没有独立的关键词表——所以认识新说法**只有"加例句"一条路**，加完不需要改任何匹配逻辑。
改完跑 `python scripts/probe_routing.py` 做 A/B 对比。
...
跑 [§11.5](#115-运行测试与门禁) 的两条门禁（`pytest` + `ruff`），并确保
`python scripts/fix_doc_linenos.py` 报告 `待修 0 处`。

> 一段有用的经验：本项目的测试里有一批**反向验证**用例（把修复退回，断言护栏变红）。
> 它们会临时往 `app/rag/` 写一个 `_layering_probe.py` 探针并在 `finally` 里删除。
> 如果 pytest 被**中断**（Ctrl-C、沙箱拦截），探针可能残留——残留会让
> `test_deadcode` / `test_layering` 一片红，且 `fix_doc_linenos.py --write`
> 会把探针的行数也统计进文档（写出一个偏大的数字）。遇到这种情况先删掉它再重跑。

---

## 关于许可

本仓库未附加开源许可文件，**默认保留全部权利**。如需在贵司内部使用或二次分发，
请先与作者联系。
