...
| 全链路自测 | 启动执行本地轻量自检（9 项，不真实调用模型），深度检查（真实调用 LLM/Embedding）经 `/test/all` 手动触发；另提供 565 项 pytest 回归（含 80 项多 Agent 架构级用例） |
