...
| 能力 | 说明 |
|---|---|
| 多 Agent 协作 | **路由 Agent 是唯一入口**：一次模型调用判定 `smalltalk` / `out_of_scope` / `simple_rag` / `complex_rag` / `tool` 五选一。职责单一带来的收益是**失败模式局部化**——闲聊挂了不影响检索，工具挂了不影响问答 |
...
| 全链路自测 | 启动执行本地轻量自检（9 项，不消耗模型配额），深度检查（真实调用 LLM/Embedding）经 `/test/all` 手动触发；另提供 460 项 pytest 回归（含 76 项多 Agent 架构级用例） |
...
服务启动时会自动执行本地轻量自检（探活基础设施，**不真实调用 LLM**，避免消耗配额与卡启动），并在知识库为空时**自动完成首次建索引**，开箱即用。深度自检（含真实模型问答）请访问 `/test/all`。
...
```bash
# 主回归套件（344 项，纯 pytest，不需要起服务）
pytest                                   # 全量
pytest tests/test_multi_agent.py -q      # 只跑多 Agent 架构级用例（76 项）
...
| `LLM_TEMPERATURE` | 0.1 | 低温度约束，保证回答严谨（kimi 系列仅接受 1，需置 1） |
| `LLM_TIMEOUT` | 60 | 单次 LLM 请求超时（秒），过长会让慢请求迟迟不失败 |
| `LLM_HEALTH_CHECK` | false | 启动时是否对真实模型做 ping 校验（关闭则把配额留给真实对话） |
