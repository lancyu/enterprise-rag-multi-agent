...
# 关闭（默认）：跳过校验、直接判定为真实模式，把限频账号（如免费档 RPM 极低，常见 3 RPM）的
#   宝贵配额留给真实对话，避免启动自检就把额度刷爆导致用户提问时触发 429。
# 开启：提前发现 Key / 模型名错误并优雅降级为 Mock；但会额外消耗 1~2 次调用。
...
# ============================================================
# 工具决策最多几轮。默认 1：模型看到工具清单后做一次决策，可在这一轮内
# **并行发出多个工具调用**（「查年假」+「查报销」各一条），覆盖绝大多数需求，
# 且总模型调用数最少。
# 设为 2 以上可支持「先查 A 才知道去哪查 B」的顺序多跳，代价是每轮多一次
# 模型调用，且最后一轮模型自己写的文本会被 L4 覆盖（最终答案必须带引用编号）。
#
...
TOOL_AGENT_MAX_STEPS: int = int(_env("TOOL_AGENT_MAX_STEPS", "3"))
...
#
# 口径：词面原始加权分 = Σ len(命中关键词)。也就是"至少命中一个三字词、
# 或两个二字词"才过线。为什么不用 2.0（一个二字词）：单个泛化词太容易误命中——
# "公司有哪些部门"只命中 employee_attr 的"部门"（2.0），若过线就会把制度类
# 问题误送进工具通道；而"张三在哪个部门"由**结构化锚点**在层① 拦下，
# 根本不需要走词面。宁可多升一次语义，不可错得干脆。
ROUTE_LEXICAL_FLOOR: float = float(_env("ROUTE_LEXICAL_FLOOR", "3.0"))
...
ROUTE_LEXICAL_MARGIN: float = float(_env("ROUTE_LEXICAL_MARGIN", "2.0"))
