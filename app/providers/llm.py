...
**免费档账号极低 RPM（3 次/分钟）** 写的，代价有两块，都是实打实的：
...
换用不限流模型后，这层包装只剩成本、没有收益，故整体移除：模型调用**直连
``ChatOpenAI``**，function calling 走**原生实现**，重试交回 SDK 的 ``max_retries``。
少一层包装，就少一处"同一语义两处各定义一遍"的土壤。

唯一保留的是 ``_is_rate_limit``：它是**纯判断函数**（零成本、无副作用），
自检模块仍用它区分「限流导致的失败」与「真正的配置错误」。换回限流模型时，
这个区分立刻又有用——保留一个纯函数，比保留一整套重试机制便宜得多。
"""
import re
