...
| **C** | **门禁只守「行号对得上」，不守「话是对的」** | 行号校验器把 598 条声明守到全绿的同时，文档里赫然写着**一个不存在的函数名**（以 `_route_edge` 结尾，真实名是 `error_route_edge`），并把 3 条条件边说成 4 条——**语义一致性没有任何机制把关** |
...
- **证据**（都已核实）：
  - `docs/project-introduction.md:168` 点名了一个**代码里不存在的函数名**
    （以 `_route_edge` 结尾那个，实际是 `error_route_edge`）；**✅ 已修**
...
- **方向**：① 先统一口径（建议以代码为准：9 节点 / 3 条件边）；
  ② **给校验器加第 14 类：文档里出现的、以 `_edge` / `_node` 结尾的符号名，必须在代码里真实存在**
  （这正好补上「行了号对了、名字却不存在」这个语义盲区；实现成本低，因为 AST 索引现成）。
- **验收**：第 14 类上线后，把那个不存在的旧函数名写回文档，校验器必须变红；
