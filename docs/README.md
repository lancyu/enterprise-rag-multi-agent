...
| [`intent-routing-hybrid-design.md`](intent-routing-hybrid-design.md) | 四层混合路由**设计稿**：惰性升级、时间预算、不谎报三字段（见下方告示） | 理解路由判据**为什么**这么定 |
...
> ⚠️ **`intent-routing-hybrid-design.md` 是「实施前」的设计稿，不是现状描述。**
> 它的**漏斗设计**（惰性升级 / 时间预算 / 门控判据）与代码一致，但两处会误导：
> ① 它反复提到的字段 `scene_capability` **最终没有落地**——实现成了 `scene`
> （答"该由谁干"）+ `intent_capability`（答"答案怎么来的"）；
> ② 文中的行号与执行预期是写作当时的快照，`verify_doc_linenos.py` 对它有 3～6 处
> 报错（它自己把它解释成"校验器误报"，那是**加固前**的结论，今天仍会报）。
