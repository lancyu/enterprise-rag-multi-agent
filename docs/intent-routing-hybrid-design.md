...
> **本文是待评审设计，尚未改动任何业务代码。**
> 文中所有"现状"判断都标注了 `文件:行号`，可直接核对；所有开源结论都来自**实际读源码**
...
| 第 2 关 | 给每个去处打个分：关键词 + 意思相近度 | 一次向量计算 |
...
校验 docs/intent-routing-hybrid-design.md：共 6 条行号声明
❌ 发现 3 处不一致：
   ✗ app/core/prompts.py:85-128
   ✗ app/graph/state.py:65-70
   ✗ app/graph/nodes.py:154-157
```

**这 3 处经核对全部为「引用正确、校验器误报」**，原因是该脚本的行号正则
（`scripts/verify_doc_linenos.py:108`）要求 `app/...py:起-止` 且**必须精确命中一个
函数/类符号**；而这 3 处分别落在「字典字面量里的提示词字符串」「TypedDict 的字段声明」
「函数体内的赋值语句」，**都不是符号**，所以索引不到。

⚠️ **一个应当记录在案的覆盖缺口**：该正则只识别**全路径 + 区间**形式，因此本文大量
`router_agent.py:107-114` 这类短路径引用**根本没被识别**（本次只识别出 6 条，
而全文有 48 条区间引用）。而这个项目的关键设计论证恰恰大量写在 **docstring 与模块级
常量**里——两者都不在符号索引内。
...
app/core/router_agent.py               ← 保留为**门面**（facade），内部改为调用 routing 包
app/api/routing.py                     ← 新增：预演接口（约 90 行）
```
...
class IntentSpec:
    ...
    utterances: tuple[str, ...]   # 语义锚点（8~15 条真实问法）
    keywords: tuple[str, ...]     # 词面锚点（**专名优先**：制度名、部门名、字段名）
    guard: str | None = None      # 命中即该能力清零（例如 howto 句式对 employee_attr）
    anchor: str | None = None     # ① 层的确定性锚点函数名（可解析，启动期校验）
...
- **为什么 RRF 而不是加权求和**：词面分（关键词命中率，0~1 但与句长相关）与语义分
...
- **词面分**：`keywords` 命中数 / 总命中数，加权（长专名权重更高）。
  专名靠词面远比向量准——"张三"与"张伟"在向量空间几乎重合。
...
**不能共用一套**——量纲不同（加权命中分 vs 余弦），共用必然一边恒过、一边恒不过。
...
**代价与边界**：词面层只有命中关键词才算分，覆盖不到换说法的问法——
这正是它后面必须跟着语义层的原因。惰性升级**不是**用词面替代语义，
...
| 项 | 预估 | 实际 | 差异原因 |
|---|---|---|---|
| `app/core/routing/` | ~870 行 | **1809 行** | 预估只算了"一条主干"；实际每个模块都带**为什么这么做/不这么做**的设计注释，且 ②③④ 一并实现了 |
| 预演接口 | ~90 行 | **110 行** | 接近 |
| `app/config.py` | 未列 | **+100 行** | 阈值、预算、超时都要能配；每条默认值都写了来由 |
| `app/core/prompts.py` | 未列 | **+19 行** | 层④ 的仲裁提示词（候选清单从目录渲染） |
| 图 / 条件边 | 0 | **0** ✅ | 符合预估 |
...
| `employee_attr` | `tool` | 索取**某个具体人的某个属性值**：部门、岗位、邮箱、分机号、主管、入职时间 | `anchor_person_attr` | `howto`, `policy_context`, `comparison` |
| `leave_balance` | `tool` | 索取**某个具体人的假期余额**：年假、调休、加班剩余 | — | `howto`, `policy_context`, `comparison` |
...
### A.2 `keywords`（词面锚点，**专名与字段名优先**）

| name | keywords |
|---|---|
| `chitchat` | （**留空**——寒暄靠整句锚定，词面会给"好像这个制度不太清楚"这种句子误加分） |
| `identity` | 你是谁、你是什么、你能做什么、你会什么、介绍一下你 |
| `employee_attr` | 工号、部门、岗位、职位、职级、邮箱、分机、主管、领导、直属、入职时间、花名 |
| `leave_balance` | 年假、调休、事假、病假、假期余额、还剩几天、剩余、可用天数 |
| `policy_single` | 制度、规定、流程、政策、年假、报销、考勤、密码、试用期、加班、出差、权限申请 |
| `policy_compare` | 对比、区别、差异、有什么不同、哪个更、冲突、分别、同时适用 |
| `redline_jailbreak` | 忽略、无视、扮演、角色、越狱、脱离设定、现在你是 |

> 注意 `policy_single` 与 `leave_balance` **都含"年假"**——这不是冲突，是**这正是要靠门控区分的地方**：
> 「年假有多少天」（制度）vs「张三的年假还剩几天」（某人的余额）。
> 词面分打平 → 但两者**跨通道**（`simple_rag` vs `tool`）→ 正确进灰区。
> 而 `employee_attr` 的 `anchor` 会在层① 就把「张三的年假还剩几天」拦下——
> **锚点与词面在这里是互补的，不是重复的。**
...
| 张三在哪个部门 | `policy_context` guard | `employee_attr`（锚点应当**先于** guard 生效） |
| 哪个部门负责报销 | `employee_attr`（含"部门"） | `simple_rag` |
| 怎么申请邮箱扩容 | `employee_attr`（含"邮箱"） | `simple_rag` |
| 年假和调休有什么区别 | `leave_balance`（"年假"2 + "调休"2 **累加成 4.0**，压过"区别"的 2.0） | `policy_compare`（靠 `comparison` guard 清零，见 §A.4） |
| 年假有多少天 | `leave_balance`（含"年假"） | `simple_rag` |

> 上表最后三行就是 §4.3 那条教训的**回归用例来源**：判据必须是
> **"属性词处于被索取位置"**（句尾、且与"哪个/什么"相邻），
> 而不是"句中出现某个词"。仅靠关键词表区分不了"提到某个词"与"要取这个值"。

### A.4 四个 guard 的判据（**判据只描述句式，不描述意图**）

| guard | 判据 | 作用于 |
|---|---|---|
| `howto` | 句式判定为"操作/流程问句"：含 `怎么 / 如何 / 怎样 / 在哪申请 / 流程`，且**主语不是具体人名** | 把 `employee_attr` / `leave_balance` 清零 |
| `policy_context` | 句中含制度名词（制度/规定/流程/政策），且**不含人属性索取结构** | 把 `employee_attr` / `leave_balance` 清零 |
| `comparison` | 句式判定为"比较两个对象"：含 `对比 / 区别 / 差别 / 差异 / 有什么不同 / 哪个更 / …`，且**不是在向某人索取属性** | 把 `employee_attr` / `leave_balance` 清零 |
| `business_noun` | 句中含**任何**业务名词（制度、流程、年假、报销、部门、员工、工号…） | 把 `redline_jailbreak` 清零 |

**`comparison` 是唯一一个由实测漏判倒逼出来的 guard**（其余三个在建目录时就写下了）。
「年假和调休有什么区别」曾被判进 `leave_balance`：词面分是 `Σ len(关键词)`，
`年假`(2) + `调休`(2) = 4.0，压过了真正表达提问意图的 `区别`(2)——
**"提到两个宾语"比"在问两者的关系"得分更高**。这是加权求和打分的固有特性，
...
所以 `comparison` 的词表与 `policy_compare` 的关键词**有重叠却必须各写一份**——
...
见 §4.3），而 guard（`policy_context` 这类）大多只是**词面共现**。
