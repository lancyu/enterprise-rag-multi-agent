...
14. **文档点名的符号必须真实存在**：`` `error_route_edge` `` / `` `tool_node` ``
    这类以 `_edge` / `_node` 结尾的名字，必须在 AST 索引里找得到。
    **这是唯一一类不看数字的声明** —— 前 13 类守的都是「数字对不对」，
    而这一条守的是「名字存不存在」。实测：文档里写着 `answer_route_edge`
    （真实名 `error_route_edge`）很久，同期校验器报的是「全部一致」。
    读者的损失是实在的：照着名字去搜，搜不到，最后怀疑的是自己。
    只挑这两个后缀是刻意的：放宽到所有反引号标识符会大面积误报
    （配置项、字段名、外部库的名字都不在本仓库的 AST 里）。

第 3～14 类都是**后来补齐的盲区**。共同的教训是：
...
先找找源码里还有没有别的稳定锚点。**
第 14 类换的又是另一个维度：前 13 类**全都是关于数字的**，
而它守的是**名字**。「行号全对」与「话是对的」是两件事 ——
600 条声明全绿的同时，文档里可以写着一个不存在的函数。**门禁守什么，
取决于你以为它在守什么；以为它在守"文档是对的"，那是最危险的误解。**
...
#: 第 14 类：**文档点名的符号必须真实存在**。只认这两个后缀，理由见 `verify()` 里本类
#: 的注释 —— 它们正是「读者最可能去搜、写错了却最不容易察觉」的一类名字。
#:
#: 首字符必须是**字母**：`` `_route_edge` `` 这种写法是在说「以什么结尾」，
#: 不是在点名一个符号（实测：写「以 `_route_edge` 结尾的那个名字」时误报过）。
_SYMBOL_NAME_RE = re.compile(r"`([a-z][a-z_0-9]*_(?:edge|node))`")
...
def verify(doc_path: str) -> list[str]:
    ...
    # --- 文档点名的符号必须真实存在（第 14 类，唯一不看数字的一类）---
    # 前 13 类守的是「数字对不对」，本类守的是**名字存不存在**：
    # 行号全绿的同时，文档里可以赫然写着一个代码里根本没有的函数，
    # 读者会照着去搜、搜不到，最后怀疑的是自己。
    # 实测就是这样：`answer_route_edge`（真实名是 `error_route_edge`）在文档里
    # 活了很久，而当时校验器报的是「598 条声明全部一致」。
    #
    # 判据：**反引号里的、以 `_edge` / `_node` 结尾的符号名**必须在 AST 索引里存在。
    # 为什么只挑这两个后缀：它们恰好是「读者最可能去搜、而写错了最不容易察觉」
    # 的一类（条件边函数与节点函数）；放宽到所有反引号标识符会大面积误报
    # （配置项、字段名、外部库名字都不在本仓库的 AST 里）。
    #
    # ⚠️ 已知的误报面：记录「已删除的符号」的文档（迁移说明、删除项表）里
    # 会写到已经不存在的名字 —— 那种地方**不要把名字写成反引号形态**，
    # 或者干脆把这类文档排除在门禁之外（本类只跑你指定的那一份文档）。
    known_names = {name for f in symbols.values() for name in f}
    known_names |= {name.rsplit(".", 1)[-1] for name in known_names}
    for lineno, line in enumerate(lines, 1):
        for m in _SYMBOL_NAME_RE.finditer(line):
            name = m.group(1)
            counted += 1
