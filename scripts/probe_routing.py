...
用法
----
零成本跑法（**推荐，零 LLM、零 embedding**）——关掉语义层与仲裁层，
只跑 ① 锚点 + ②a 词面。这两层纯本地计算，不发任何网络请求，
所以结果**可重复**：
...
全开（灰区会真实调用 LLM 仲裁，**有真实耗时与成本，结果会被上游排队噪声污染**）：
...
读结果时注意两件事
------------------
1. **别把上游排队当成代码 bug**：供应商超限时常常**不报 429**——
   服务端 hold 住连接等窗口，表现为首字延迟被拉到 10~17 秒、日志完全干净。
   所以全开模式下"某条忽然慢了/判歪了"先怀疑上游，不要先翻代码。
